...
function getUser($size) {
	...
	$sql="select onchba_sn,onchba_cycle,onchba_size,onchba_tsize from online_change_basin where onchba_status > 0 and onchba_size like '{$size}' order by onchba_size, onchba_tsize";
...
function getFacilityAllocation() {
	$ret_data = array();
	$conn = getDB();

	$sql="SELECT * FROM `js_config` WHERE jsc_key like 'FacilityAllocation'";
