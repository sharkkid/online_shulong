...
if(!empty($op)) {
	...
	$product_list = getProducts($search_where, $pg_offset, $pg_rows);
	// printr($product_list);
	// exit;
}
?>
						...
						<form autocomplete="off" method="get" action="./plant_purchase_details1234.php" id="search_form" class="form-inline alert alert-info" role="form">
							<div class="row">
								<div class="col-md-12">
									...
									<div class="form-group">
										<label for="searchInput4">尺寸</label>
										<select class="form-control" name="onproduct_size">
											<option value="" selected="selected">全部</option>
											<?php												
												foreach ($Size as $key => $value) {
													if($onproduct_size == $key)
					...
					<table class="table table-striped table-hover table-condensed tablesorter">
        				<thead>
        					<tr style="font-size: 1.1em">
        						<th class="product_title" style="text-align: center;">照片</br>(Photo)</th>
        						<th class="product_title" style="text-align: center;">品號</br>(Part no.)</th>
        						<th class="product_title" style="text-align: center;">品名</br>(Part name)</th>        						
        						<th class="product_title" style="text-align: center;">數量</br>(只統計苗株)</th>
        						<th class="product_title" style="text-align: center;">#</th>
        					</tr>
        				</thead>
        				<tbody>
        					<?php        					
        					foreach ($product_list as $row) {
        						if($row['onadd_cur_size'] != ""){
	        						echo '<tr>';
	        							if($row['onproduct_pic_url'] != '')
	        								echo '<td style="border-right:0.1rem #BEBEBE dashed;text-align: center;vertical-align: middle;"><img src="'.$row['onproduct_pic_url'].'" style="width: 9.5em;"></td>';
	        							else
	        						...
	        						// echo '<td><button type="button" class="btn btn-info btn-xs" onclick="location.href=\'			./details_table1234.php?onproduct_sn='.$row['onproduct_sn'].'&onproduct_part_no='.$row['onproduct_part_no'		].'	&onproduct_growing='.$row['onproduct_growing'].'&onadd_quantity_del='.date('Y').'\'		">查看</button></td>';
	        							echo '<td class="product_title" style="border-right:0.1rem #BEBEBE dashed;text-align: center;vertical-align: middle;">'.$row['onproduct_part_no'].'</td>';//品號
	        							echo '<td class="product_title" style="border-right:0.1rem #BEBEBE dashed;text-align: center;vertical-align: middle;">'.$row['onproduct_part_name'].'</td>';//品名
	        							
	        							echo '<td class="product_title" style="text-align: center;vertical-align: middle;">'.$row['sum'].'</td>';//數量
	        							echo '<td class="product_title" style="text-align: center;vertical-align: middle;"><button type="button" class="btn btn-info btn-xs" onclick="location.href=\'/online_shulong/admin/purchase/details_table.php?onadd_part_no='.$row['onproduct_part_no'].'&onadd_quantity_del='.Date('Y').'&onadd_part_name='.$row['onproduct_part_name'].'\'"><i class="glyphicon glyphicon-plus"></i> 預訂</button></td>';
	        						echo '</tr>';
	        					}
        					}
        					?>
