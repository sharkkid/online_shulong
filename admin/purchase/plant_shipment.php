    ...
    // 塞值
        ...
        for($i=0;$i<count($User_forExcel);$i++){
            ...
            if ($User_forExcel[$i]['onadd_type'] < 2) { 
            	$total_revenue = round($User_forExcel[$i]['real_price']*$User_forExcel[$i]['onshda_quantity'],2);
        		$total_cost = round($User_forExcel[$i]['cost_money']*$User_forExcel[$i]['onshda_quantity'],2);
        		if(!empty($User_forExcel[$i]['onshda_real_revenue'])){
					$total_revenue = round($User_forExcel[$i]['onshda_real_revenue'],2);
        		}
        		$total_profit = $total_revenue - $total_cost;
            	//總收入
            	$sheet->setCellValue('H'.($n+$i), $total_revenue);
            	//總成本
        		$sheet->setCellValue('I'.($n+$i), ($User_forExcel[$i]['cost_money']+$User_forExcel[$i]['total_cost_shipment']+$User_forExcel[$i]['onadd_other_price'])*$User_forExcel[$i]['onshda_quantity']);
	            //毛利
								...
								<div class="row">
									<div class="col-md-12">
										...
										<div class="form-group">
											...
											<div class="col-md-10">
												<input type="text" class="form-control" id="onshda_add_date" name="onshda_add_date" placeholder="" required 	minlength="1" maxlength="32">
												<div class="help-block with-errors"></div>
											</div>
        								...
        								<div class="form-group">	
											...
											<div class="col-md-1" style="">	
												<input type="text" class="form-control" id="onriadd_other_item" name="onriadd_other_item" placeholder="項目" minlength="1" maxlength="32" style="border-bottom: 1px solid rgba(0, 0, 0, 0.6);border-top: 0px;	border-right: 0px;border-left: 0px;height: 28px;padding: 0px;text-align: center;padding-left: 0px;	padding-right: 0px;">
											</div>
										...
										<div class="form-group">	
											...
											<div class="col-md-10">
												<input type="text" class="form-control" id="addModalInput1" name="onshda_real_price" placeholder="" required 	minlength="1" maxlength="32">
												<div class="help-block with-errors"></div>
											</div>
										</div>  					
									</div>
        					...
        					<tbody>
        						...
        						foreach ($ship_list as $row) {
        							...
        							if(!empty($row['onshda_real_price'])){
        								echo '<td style="text-align: center;">是</td>';//實際價格
        							}
        							else{
        								echo '<td style="text-align: center;">否</td>';//實際價格
        							}

        							// if($row['onshda_real_price'] == "是")
        							// 	echo '<td style="text-align: center;"></td>';
        							// else
        								echo '<td style="text-align: center;"><button type="button" style="background-color:#f67828;border:#f67828" class="btn btn-danger btn-xs adjust" data-onshda_sn="'.$row['onshda_sn'].'" data-sn="'.$sn.'" data-cost_money="'.$row['cost_money'].'">編輯</button></td>';
        							echo '</td></tr>';
        						}
        						?>
        					</tbody>
