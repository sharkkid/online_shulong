...
if(!empty($op)) {
	...
	switch ($op) {
		...
		$onproduct_add_date=GetParam('onproduct_add_date');//建立日期
		$onproduct_mod_date=GetParam('onproduct_mod_date');//修改日期
		...
		$onproduct_quantity_cha=GetParam('onproduct_quantity_cha');//換盆數量
		$onproduct_quantity_cha123 =($onproduct_quantity - $onproduct_quantity_cha);
		if($onadd_quantity_cha123<=0) {
			$onadd_status = -1;
		} else {
			$onadd_status = 1;
	...
	* {
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			<?php
					//	init search parm
			// print "$('#search [name=onproduct_status] option[value={$onproduct_status}]').prop('selected','selected');";
			// print "$('#search [name=onproduct_growing] option[value={$onproduct_growing}]').prop('selected','selected','selected','selected','selected','selected','selected');";
			?>
		...
		$.ajax({
			...
			$('button.del').on('click', function(){
				onproduct_sn = $(this).data('onproduct_sn')
			...
			$('#add_form, #upd_form, #upd_form1, #upd_form2, #upd_form3').validator().on('submit', function(e) {
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div id="search" style="clear:both;">
						<form autocomplete="off" method="get" action="./plant_purchase_add.php" id="search_form" class="form-inline alert alert-info" role="form">
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
