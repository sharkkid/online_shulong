...
if(!empty($op)) {
	...
	switch ($op) {
		...
		case 'export_all':	
		    ...
		    // init excel
		    $inputfilename = WT_PATH_ROOT.'/admin/purchase/customer_log_temp.xls';
		    ...
		    $sheet->freezePane('A3');
		    $sheet->setTitle("客戶管理紀錄_".date("Y-m-d"));
		    ...
		    foreach ($ship_list as $key => $value) {
		    	foreach ($value as $key2 => $value2) {
			        ...
			        //交易日期
			        $sheet->setCellValue('B'.($n+$i), date('Y-m-d',$value2['onshda_add_date']));
			        //品號
			        $sheet->setCellValue('C'.($n+$i), $value2['onadd_part_no']);
			        //品名
			        $sheet->setCellValue('D'.($n+$i), $value2['onadd_part_name']);
			        //交易數量
			        $sheet->setCellValue('E'.($n+$i), $value2['onshda_quantity']);
			        //育成率
		...
		case 'export_individual':	
			if($onshda_client = GetParam('customer')){
			    ...
			    $sheet = $originalexcel->getSheetByName($sheetname);
			    $sheet->freezePane('A3');
			    $sheet->setTitle("客戶管理紀錄_{$onshda_client}_".date("Y-m-d"));
			    ...
			    // 產生檔案
			    $excelextend = substr($inputfilename, strpos($inputfilename, "."));
			    $filename="客戶管理紀錄_{$onshda_client}_".date("YmdHis");
			    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			    header("Content-Disposition: attachment;filename=".$filename.$excelextend);
			    header('Cache-Control: max-age=0');
			    // $objWriter = PHPExcel_IOFactory::createWriter($originalexcel, 'Excel2007');
	...
	if(($onshda_client = GetParam('customer')) && $onshda_client != '1') {
	...
	// printr($search_where);
	$ship_list = getCustomerSoldLog($search_where);
	$_SESSION['ship_list'] = $ship_list;
	$customer_list = getAlreadySoldCustomer();
	// printr($search_where);
	// exit;
	// printr($ship_list_2);
}
...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			if(!empty($onshda_client))
				print "$('#search [name=customer] option[value={$onshda_client}]').prop('selected','selected','selected','selected','selected','selected','selected');";
			?>
			...
			$('#adjust_form').validator().on('submit', function(e) {
				...
				$('button.export_excel').on('click', function(){
            	    window.open("plant_shipment_customer_log.php?op=export_all");
	
            	});

            	$('button.export_individual').on('click', function(){
            		var customer = $(this).data('customer');
            		window.open("plant_shipment_customer_log.php?op=export_individual&customer="+customer);
            	});
		});
	</script>	
</head>
        		...
        		<div class="row">
        			<div class="col-md-12">
        				...
        				<div id="search" style="clear:both;">
        					<form autocomplete="off" method="get" action="./plant_shipment_customer_log.php" id="search_form" class="form-inline alert alert-info" role="form">
        						<div class="row">
        							<div class="col-md-12">
        								...
        								<div class="form-group">
        									<label for="searchInput4">客戶</label>
										...
										<table class="table table-hover table-condensed tablesorter">
				        					...
				        					<tbody>
				        						<?php
				        							...
				        							foreach ($value as $key2 => $value2) {
				        								echo '<tr>';
				        								echo '<td style="vertical-align: middle;text-align: center;">'.Date('Y-m-d',$value2['onshda_add_date']).'</td>'; 
				        								...
				        								echo '<td style="vertical-align: middle;text-align: center;">'.$value2['onshda_quantity'].'</td>'; 
				        								echo '<td style="vertical-align: middle;text-align: center;">'.$value2['livability'].'</td>'; 
				        								echo '</tr>';
				        							}
