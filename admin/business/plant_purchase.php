<?php
include_once("../purchase/func_plant_purchase.php");
...
if(!empty($op)) {
	...
	switch ($op) {
		...
		if(empty($onadd_planting_date)||empty($onadd_quantity)||empty($onadd_growing)){
			...
			if($onadd_status != -1) {
			    ...
			    if ($onadd_type == 1) {
					...
					// 上次換盆成本價格    
				    $sql_total_last_cost = "SELECT onadd_buy_price FROM onliine_add_data where onadd_sn='{$onadd_sn}'";
				  ...
				  //   // 累計加總
				    ...
				    $sql = "INSERT INTO onliine_add_data (onadd_add_date, onadd_mod_date, onadd_part_no, onadd_part_name, onadd_color, onadd_size, onadd_height, onadd_pot_size, onadd_supplier, onadd_planting_date, onadd_quantity,onadd_buy_price,onadd_quantity_cha, onadd_growing, onadd_status, jsuser_sn, onadd_cycle, onadd_newpot_sn, onadd_cur_size, onadd_location, onadd_sellsize, onadd_next_status, onadd_plant_staff, onadd_price_per_plant, onadd_other_price, onadd_type, onadd_level) " .
		...
		if(empty($onadd_plant_year)){
			...
			$sql2 = "INSERT INTO `online_shipment_data` (onshda_add_date, onshda_mod_date, onshda_client, onshda_quantity, onadd_sn, onadd_part_no, onadd_part_name, onadd_data_sn) VALUES ('{$onshda_add_date}','{$now}','{$onshda_client}','{$onadd_plant_year}','{$sn}','{$onadd_part_no}','{$onadd_part_name}','{$onadd_data_sn}');";				
						 	...
						 	$.ajax({
						 		...
						 		success: function(ret) {
						 			if(ret.msg != '出貨完成！' && ret.msg != '換盆成功！'){
						 				...
						 				if(ret.msg == '出貨完成！'){
						 					quantity = param[8]['value'] - param[10]['value'];
						 				}
						 				else{
		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12">
					...
					<!-- content -->
					<ul class="nav nav-tabs" style="font-size: 1em">
						<li class="active" ><a style="color:#242028;" href="<?php echo WT_URL_ROOT;?>/admin/business/plant_purchase.php">固定成本</a></li>
						<li class="active"><a style="color:#242028; background-color: #dcdcdc;" href="<?php echo WT_URL_ROOT;?>/admin/business/plant_purchase_float.php">浮動成本</a></li>
					</ul>
					<table class="table table-striped table-hover table-condensed tablesorter">
						<thead>
							<tr style="font-size: 1.1em">
								...
								<th style="text-align: center;">下階段換盆尺寸</th> 
								<th style="text-align: center;">預計出貨規格</th> 
								<th style="text-align: center;">預計成熟日/出貨日</th>
								<th style="text-align: center;">已種植週數</th>		
								<th style="text-align: center;">累積成本(每株)</th> 
								<?php 
									$flag = false;
									for($i=1;$i<7;$i++){
						...
						<tbody >
							...
							foreach ($product_list as $row) {								
								echo '<tr>';//產品編號
        							...
        							//預計成熟日     							
        							echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$row['pre_days'].'</td>';     							
        							//已種植週數
	        						$OriginalSn = GetOriginalSn($row['onadd_sn'], $row['onadd_type'], $row['onadd_newpot_sn'], $row['onadd_ml'], $row['onadd_planting_date'], $row['onadd_AB_sn'], $row['onadd_level']);
	        						$PlantWeeks = HowManyWeeksPlant(Date('m/d/Y',time()),Date('m/d/Y',GetFirstPlantDay($OriginalSn)));
	        						if($PlantWeeks == "0")
	        							$PlantWeeks = 1;

	        						echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$PlantWeeks.'周</td>';
	        								        					
        							if($row['onadd_type'] == 2){
        								// //每周成本計算
	        							// $SellSizeWeeks = GetSellSizeWeeks($row['onadd_sellsize']);
	        							// if($SellSizeWeeks != 0){
	        							// 	$CostPerWeek = round(($row['onadd_buy_price']/$SellSizeWeeks)*$PlantWeeks , 2);
	        							// 	echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$CostPerWeek.'元</td>';
	        							// }
	        							// else{
	        								echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;"></td>';
	        							// }		
	        						...
	        						// 自種苗
        								...
        								echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$row['CostBase'].' 元</td>';   
        							}
