...
function get_SUM_CostTable() {
	$ret_data = array();
	$conn = getDB();
	$sql="select * from onliine_cost_sum_table where oncost_status = 1 ORDER BY oncost_sn DESC";
...
function get_SUM_CostDetail($oncost_sn,$oncoda_cost_size) {
	$ret_data = array();
	$conn = getDB();
	$sql="select * from online_cost_sum_data where oncoda_status = 1 and oncost_sn = {$oncost_sn} and oncoda_cost_size = $oncoda_cost_size";
