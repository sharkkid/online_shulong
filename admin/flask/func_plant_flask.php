...
function getProductByPartNo($onproduct_part_no) {
	$ret_data = array();
	$conn = getDB();
	$sql="select * from onliine_product_data a left join onliine_add_data b on a.onproduct_part_no = b.onadd_part_no where onproduct_part_no='{$onproduct_part_no}' order by onadd_sn DESC LIMIT 0 , 1";
...
function IsProductExit($onproduct_part_no,$onproduct_part_name,$onproduct_plant_st='1') {
	$ret_data = "0";
	$conn = getDB();
	$sql="select * from onliine_product_data where onproduct_part_no='{$onproduct_part_no}' and onproduct_part_name='{$onproduct_part_name}' and onproduct_plant_st = '{$onproduct_plant_st}'";
...
function getPlantDataBysn($onadd_sn) {
	$ret_data = array();
	$conn = getDB();
	$sql="select * from onliine_add_data where onadd_sn='{$onadd_sn}'";
	...
	if ($qresult->num_rows > 0) {
		if ($row = $qresult->fetch_assoc()) {
			if(!empty($row['onproduct_pic_url'])){
				$ret_data['img_url'] = "./../../admin/purchase/".substr($row['onproduct_pic_url'], 1);
				$ret_data['preview_img_url'] = "./".substr($row['onproduct_pic_url'], 1);
			}
			else{
				$ret_data['img_url'] = "./../../admin/purchase/images/nopic.png";
				$ret_data['preview_img_url'] = "";
			}
		}
		$qresult->free();
	}else
...
function getAllSupplier() {
	$ret_data = array();
	$conn = getDB();
	$sql="SELECT onadd_supplier,onadd_supplier_phone,onadd_supplier_address FROM `onliine_add_data` GROUP BY onadd_supplier";	
	$qresult = $conn->query($sql);
	if ($qresult->num_rows > 0) {
		while($row = $qresult->fetch_assoc()) {				
			$ret_data[0][] = $row['onadd_supplier'];
			$ret_data[1][] = $row['onadd_supplier_phone'];
			$ret_data[2][] = $row['onadd_supplier_address'];
		}
		$qresult->free();
	}
	$conn->close();
...
function getSupplierByName($onadd_supplier) {
	$ret_data = array();
	$conn = getDB();
	$sql="SELECT onadd_supplier_phone,onadd_supplier_address,onadd_supplier_email FROM `onliine_add_data` WHERE onadd_supplier = '{$onadd_supplier}'  GROUP BY onadd_supplier";	
	$qresult = $conn->query($sql);
	if ($qresult->num_rows > 0) {
		while($row = $qresult->fetch_assoc()) {					
			$ret_data = $row;
...
function getAllPantStaff() {
	$ret_data = array();
	$conn = getDB();
	$sql="SELECT onadd_plant_staff FROM `onliine_add_data` GROUP BY onadd_plant_staff";	
	$qresult = $conn->query($sql);
	if ($qresult->num_rows > 0) {
		while($row = $qresult->fetch_assoc()) {		
			if($row['onadd_plant_staff'] != "")			
				$ret_data[0][] = $row['onadd_plant_staff'];
		}
		$qresult->free();
	}
	$conn->close();
